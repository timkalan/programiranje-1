...
let is_full_row2 (x1, x2, x3) = if (x1 = None || x2 = None || x3 = None) then false else true
...
let winner_of_triple2 triple : player option = 
    match triple with 
...
let rec winner_of_list triples : player option = 
    match triples with
    | [] -> None
    | triple :: rest -> (match winner_of_triple triple with
